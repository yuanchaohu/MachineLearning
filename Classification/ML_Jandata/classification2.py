...
def get_test_alloy(testalloy, datafile = '../finaldata.csv'):
    ...
    data[conditions].to_csv('finaldata_' + name + '.csv', index = False)
    data[~conditions].to_csv('finaldata_others.csv', index = False)
...
def Multi_classifiers():
    ...
    params_SVM = [{'C': np.logspace(-8, 5, 14), 'kernel': ['poly', 'rbf', 'sigmoid'], 'gamma': np.logspace(-8, 2, 11)}]
