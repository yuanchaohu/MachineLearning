    ...
    for p in data1.index.tolist():
        ...
        for m in compositions:
            for n in compositions:
                medium += data1.loc[p, m] * data1.loc[p, n] * heatofmixing.loc[m, n] * 4
...
def features_elements(alloys, n, file1 = './element_full.csv'):
    """ Design features of elemental properties 
    ...
    #--------------read in used elements---------------------------
    usedelements = [i for i in alloys.columns if len(i) <= 2]

    #-------------get elemental features---------------------------
    data1 = alloys[usedelements].astype(bool)
    alloys['frac1'] = np.nan
    alloys['frac2'] = np.nan
    for j in elements.columns:
        ...
        features = []
        for k in range(n):
            alloys[j + '_' + str(k + 1)] = np.nan
            features.append(j + '_' + str(k + 1))

        for i in alloys.index:
            alloys.loc[i, features] = (elements.loc[usedelements, j])[data1.loc[i]].values
            alloys.loc[i, ['frac1', 'frac2']] = ((alloys.loc[i, usedelements])[data1.loc[i]].values)[:2]

    return alloys


def element_alloy(alloys, compositions, file1 = './elements_full.csv', file2 = './modified_heatofmixing.csv'):
    """ Feature engineering for machine learning by giving 
...
#---------------elemental properties--------------------
    properties = [i for i in elements.columns if i not in ['s', 'p', 'd', 'f']]
    for m in properties:
        medium = elements.loc[compositions, m].values
        data1[m + 'Min'] = medium.min()
        data1[m + 'Max'] = medium.max()
        data1[m + 'Mean'] = medium.mean()
        data1[m + 'GeoMean'] = medium.prod() ** (1. / len(compositions))

        medium1 = medium[np.newaxis, :] * data1[compositions].values
        data1[m + 'MinMol'] = medium1.min()
        data1[m + 'MaxMol'] = medium1.max()
        data1[m + 'MeanMol'] = medium1.mean()
        data1[m + 'GeoMeanMol'] = medium1.prod() ** (1. / len(compositions))
