...
def predictions(n = 6):
    ...
    np.savetxt('coeff_n' + str(n) + '.dat', medium, fmt = '%d %.6f', comments = '', header = 'i coeff')
    x_test['Measured'] = y_test
    x_test['Predicted'] = y_predict
    x_test.to_csv('predictions_n' + str(n) + '.csv')
    ...
    plt.legend(fontsize = 14)
    plt.tight_layout()
    plt.savefig('predictions_n' + str(n) + '.png', dpi = 600)
    plt.show()
    plt.close()
...
def get_feature(n = 6):
    poly = PolynomialFeatures(n).fit(x_train_prepared)
    medium = poly.get_feature_names(x_train_prepared.columns)

    data = pd.read_csv('coeff_n' + str(n) + '.dat', sep = '\s+')
    ...
    data.to_csv('features.weights.terms_n' + str(n) + '.csv')
